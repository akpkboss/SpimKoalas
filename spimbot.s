...
.data
#from lab 9 for puzzle solution
puzzle:      .half 0:164
heap:        .half 0:4160
shouldPaint:        .byte 0:1
inventory:      .half 0:164
# syscall constants
PRINT_STRING            = 4
PRINT_CHAR              = 11
PRINT_INT               = 1
...
#stuff I added from documentation
POWERUP_MAP             = 0xffff00e0
        #unsigned short x
        #unsigned short y
        #unsigned activation_timestamp
        #PowerUpType powerUpTime
...
checkPaint:
    ...
    sw $t0, SWITCH_MODE($zero)
        ...
        #then use it
        #}
    # sw $t0, POWERUP_MAP($zero)
    #li $t5, 0
    # lw $t1, POWERUP_MAP($zero)
    # lw $t2, POWERUP_MAP($t5)
    #
    # sw $t3, BOT_X($zero)
    # sw $t4, BOT_Y($zero)
    # bne $t3, $t1, contPaintCheck
    # bne $t4, $t2, contPaintCheck
    #PICKUP_POWERUP
    ...
    li $t0, 1
    sw $t0, PICKUP_POWERUP($zero)

    la $t0, inventory
    sw $t0, GET_INVENTORY($zero)
    la $t0, inventory
    lb $t0, 8($t0)
    beq $t0, $zero, contPaintCheck
    lw $t0, GET_PAINT_BUCKETS($zero)
    beq $zero, $t0, contPaintCheck
    li $t0, 0
    sw $t0, USE_POWERUP($zero)
    li $t0, 10
    sw $t0, VELOCITY($zero)
contPaintCheck:

    lw $t0, GET_PAINT_BUCKETS($zero)
    beq $t0, $zero, getPuzzle #if the number of PAINT_BUCKETS is zero then get another puzzle
    j checkPaint
...
waitFlag:
    lbu $t0, shouldPaint($zero)
    beq $t0, 1, solvePuzzle
    j waitFlag

solvePuzzle:
    #below code from piazza - solved issues of correctly calling solve
    la $a0, puzzle
    la $a1, heap

    jal copy_board #i think this is how to save board
    ...
    #set flag & velocity back
    li $t0, 0
    sb $t0, shouldPaint($zero)
...
request_puzzle_interrupt:
    ...
    li $t0, 1
    sb $t0, shouldPaint($zero)
    j   interrupt_dispatch         #see if other interrups are waiting
